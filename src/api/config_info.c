...
extern void *slurm_ctl_conf_2_key_pairs (slurm_ctl_conf_t* slurm_ctl_conf_ptr)
{
	...
	key_pair = xmalloc(sizeof(config_key_pair_t));
	key_pair->name = xstrdup("TaskPluginTmpDirs");
	key_pair->value = xstrdup(slurm_ctl_conf_ptr->task_plugin_tmp_dirs);
	list_append(ret_list, key_pair);

	key_pair = xmalloc(sizeof(config_key_pair_t));
	key_pair->name = xstrdup("TaskPluginTmpSubdir");
	key_pair->value = xstrdup(slurm_ctl_conf_ptr->task_plugin_tmp_subdir);
	list_append(ret_list,key_pair);
